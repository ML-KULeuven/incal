...
def experiment():
    ...
    t = [[sum(n[1] for n in nearest_neighbors[i]) / len(nearest_neighbors[i]) *
          (domain.var_domains[v][1] - domain.var_domains[v][0]) if domain.is_real(v) else 0 for v in domain.variables]
    ...
    directory = "output{}lariat-synthetic{}{}".format(os.path.sep, os.path.sep, time.strftime("%Y-%m-%d %Hh%Mm%Ss"))
    os.makedirs(directory)

    name = os.path.join(directory, "combined.png")
    plot.plot_combined("x0", "x1", domain, None, (data, labels), None, name, set(), set())
    ...
    (new_data, new_labels, learned_formula), k, h = learn_bottom_up(data, labels, learn_inc, 1, 1, 2, 4, None, None)
